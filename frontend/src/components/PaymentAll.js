...
export const PaymentAll = () => {
  ...
  useEffect(() => {
    const getAllPayments = async () => {
      ...
      try{
        await axios.get('http://localhost:8090/payment/getAllPayment')
        .then((res) => {
  ...
  const handleDelete =  async(id) => {
    try{
      ...
      if(confirmed){
        await axios.delete(`http://localhost:8090/payment/deletePayment/${id}`)
        .then((res) => {
  ...
  const SearchFunction = async (searchTerm) => {
    ...
    try{
      const searchTermAsNumber = parseFloat(searchTerm);
        await axios.get('http://localhost:8090/payment/searchPayment', {
        params: {
